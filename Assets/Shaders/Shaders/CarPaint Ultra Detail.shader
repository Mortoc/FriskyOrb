...
SubShader {
   Tags { "QUEUE"="Geometry" "RenderType"="Opaque" " IgnoreProjector"="True"}	  
      Pass {  
         ...
         fixed4 frag(vertexOutput i) : COLOR
         {
			...
			fixed4 textureColor = tex2D(_MainTex, fixed2(i.tex));
 ...
 //              attenuation = 1.0; // no attenuation
            ...
            {
               fixed3 vertexToLightSource = fixed3(_WorldSpaceLightPos0 - i.posWorld);
            ...
            fixed3 halfwayVector = 
               normalize(lightDirection + i.viewDir);				  
				  
			fixed dotLN = dot(lightDirection, i.normalDir); 				  
				  
               float3 binormalDirection = 
				cross(i.normalDir, i.tangentWorld);					  
 ...
 //flakes
			...
			fixed3 vNormal = fixed3(0.5,0.5,1.0);//encodedNormal;// tex2D( _BumpMap, i.tex );
			vNormal = 2 * vNormal - 1.0;
			//fixed3 vNormal = 2 * fixed3(0.0,1.0,0.0) - 1.0;
			
			fixed3 vFlakesNormal = tex2D( _SparkleTex, i.tex.xy * 20.0 * _FlakeScale );
			//fixed3 vFlakesNormal = tex2D( _SparkleTex, i.posWorld.yz * 1.0 );
			
			vFlakesNormal = 2 * vFlakesNormal - 1.0;
			fixed3 vNp1 = vFlakesNormal + 4 * vNormal ; 
			fixed3 vView =  normalize( i.viewDir );
			
			fixed3x3 mTangentToWorld = transpose( fixed3x3( i.tangentWorld,binormalDirection,i.worldNormal ) );
   ...
   paintColor *= attenuation;
			...
			fixed SurfAngle= clamp(abs(dot (reflectedDir,i.normalDir)),0,1);
			...
			fixed4 color = fixed4(textureColor.rgb * saturate(i.vertexLighting + ambientLighting + diffuseReflection) + specularReflection, 1.0);
